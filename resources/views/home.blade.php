...
<div class="row">
  ...
  <div class="column">
    ...
    <span style="text-align: center;"><h2>Delayed Tests Today</h2></span>  
      ...
      @foreach($todayCri as $key => $todayCritical)

      <tr>
      <td>{{ $todayCritical->availableTest->name ?? '' }}</td>
        <td>{{ $todayCritical->patient->Pname  ?? '' }}</td>
        <td>{{ $todayCritical->patient->start_time  ?? '' }}</td>
        <td>
                            <button class="btn btn-xs btn-info">Progressing</button>
        </td>
        <td>
        @can('event_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('test-performed-edit', $todayCritical->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan
                                @can('event_delete')
                                    <form  method="POST" action="{{ route("performed-test-delete", [$todayCritical->id]) }}" onsubmit="return confirm('{{ trans('global.areYouSure') }}');"  style="display: inline-block;">
